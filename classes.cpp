   ...
   public:
    ...
    ThreadPool(int num_threads) {
        ...
        for (int i = 0; i < this->num_threads_; ++i) {
            pthread_create(&this->threads_[i], nullptr, thread_pool_worker, this);
        }
    }
    ...
    static void* thread_pool_worker(void* arg) {
        ...
        while (true) {
            ...
            std::function<void()> task;
            {
                ...
                pool->condition_.wait(lock,
                                      [pool] { return pool->stop_ || !pool->tasks_.empty(); });
...
class Socket {
   public:
    ...
    std::string __CLI__;
    std::string _request_file_buffer =
        "HTTP/1.1 404 OK\nContent-Type: "
    ...
    static void print(char buffer[1024]) { std::cout << "\n\n" << buffer << "\n\n" << std::endl; };

    static void print(std::string str) { std::cout << "\n\n" + str + "\n\n" << std::endl; };
    ...
    void start_many() {
        ...
        while (true) {
            ...
            this->_new_socket =
                accept(this->_server_fd, (sockaddr*)&clientAddress, &clientAddressLength);
            ...
            if (this->__CLI__.find("threaded") > 0) {
                thread_pool->add_thread(worker_thread_task, ((void*)&this->_new_socket));
    ...
    static void process_DELETE() { std::cout << "DELETING" << std::endl; };

    static void process_PUT() { std::cout << "PUTTING" << std::endl; };

    static void process_FILE() { std::cout << "FILING" << std::endl; };
    ...
    static std::map<std::string, std::string> get_POST(char buffer[1024]) {
    ...
    static std::string process_POST(char buffer[1024]) {
        std::map<std::string, std::string> url_map = get_POST(buffer);
        if (DEBUG == true) print(url_map);
    ...
    static std::string process_request(char buffer[1024]) {
        ...
        if (req_type == "GET") return Socket::process_GET(buffer);
        if (req_type == "POST") return Socket::process_POST(buffer);
        if (req_type == "PUT") Socket::process_PUT();
        if (req_type == "DELETE") Socket::process_DELETE();
        return "Not found";
    };

    static std::tuple<std::map<std::string, std::string>, std::string> get_json_from_query_string(
    ...
    static std::tuple<std::map<std::string, std::string>, std::string> split_query_string_map(
        std::string query_string) {
    ...
    static std::tuple<std::map<std::string, std::string>, std::string> get_query_string_map(
        char buffer[1024]) {
    ...
    static RequestVariables get_request_variables(char buffer[1024]) {
        ...
        request_variables.iss_line >> request_variables.method >> request_variables.url_path >>
            request_variables.http_version;
    ...
    void start(std::string url = "") {
        ...
        this->_new_socket = accept(this->_server_fd, (struct sockaddr*)&this->_address,
    ...
    void bind_socket_to_address() {
        int bound_socket =
            bind(this->_server_fd, (struct sockaddr*)&this->_address, sizeof(this->_address));
        if (bound_socket < 0) {
            perror("bind failed");
            exit(EXIT_FAILURE);
        }
