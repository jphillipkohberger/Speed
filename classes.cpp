    ...
    while (true) {
      ...
      std::function<void()> task;
      {
        ...
        pool->condition_.wait(
            lock, [pool] { return pool->stop_ || !pool->tasks_.empty(); });
...
public:
  ...
  void start_many() {
    ...
    while (true) {
      ...
      this->_new_socket = accept(this->_server_fd, (sockaddr *)&clientAddress,
                                 &clientAddressLength);
      ...
      if (this->__CLI__.find("threaded") > 0) {
        thread_pool->add_thread(worker_thread_task,
                                ((void *)&this->_new_socket));

      } else {
        thread_pool->add_task(
            [_new_socket]() { worker_thread_task(((void *)&_new_socket)); });
      }
      i++;
    }
    delete thread_pool;
  ...
  static std::string process_GET(char buffer[1024]) {
    ...
    print(buffer);
    return Socket::process_request_response(
        WEB_ROOT + Socket::get_request_data("url_path", buffer));
  };

  static std::string get_file_type(std::string file) {
    std::cout << "GETTING FILE TYPE:: " << file.substr(file.rfind(".") + 1)
              << std::endl;
    return file.substr(file.rfind(".") + 1);
  };

  static std::string get_file_name(std::string file) {
    std::cout << "GETTING FILE NAME:: " << file.substr(file.rfind("/") + 1)
              << std::endl;
    return file.substr(file.rfind("/") + 1);
  };
  ...
  static std::string get_post_data_type(std::string data_string) {
    if (data_string.find("=") != std::string::npos &&
      ...
      return "FORM";
    try {
      json parsed_json = json::parse(data_string);
  ...
  static std::string
  remove_question_from_query_string(std::string query_string) {
  ...
  split_query(std::string query_string) {
    std::map<std::string, std::string> url_map;
  ...
  static RequestVariables get_request_variables(char buffer[1024]) {
    ...
    request_variables.iss_line >> request_variables.method >>
        request_variables.url_path >> request_variables.http_version;
  ...
  static std::string get_request_data(std::string name, char buffer[1024]) {
    RequestVariables rv = get_request_variables(buffer);
    if (name == "url_path")
      return is_question_query_string(rv.url_path);
    if (name == "method")
      return rv.method;
    if (name == "http_version")
      return rv.http_version;
    if (name == "post_data")
      return get_request_body_data(&rv);
    return "";
  };
  ...
  void start(std::string url = "") {
    ...
    this->_new_socket =
        accept(this->_server_fd, (struct sockaddr *)&this->_address,
  ...
  void bind_socket_to_address() {
    int bound_socket =
        bind(this->_server_fd, (struct sockaddr *)&this->_address,
