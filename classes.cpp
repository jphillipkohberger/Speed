 ...
 public:
    ...
    ThreadPool(int num_threads) {
        ...
        for (int i = 0; i < this->num_threads_; ++i) {
            pthread_create(&this->threads_[i], nullptr, thread_pool_worker,
                           this);
...
class Socket {
 public:
    ...
    void start_many() {
        ...
        while (true) {
            ...
            this->_new_socket =
                accept(this->_server_fd, (sockaddr *) & clientAddress,
                       &clientAddressLength);
    ...
    static std::string process_GET(char buffer[1024]) {
        ...
        print(buffer);
        return Socket::process_request_response(WEB_ROOT +
                                                Socket::get_request_data
                                                ("url_path", buffer));
    };
    ...
    static std::string process_file_exists(std::string request_file) {
        ...
        if (file.good()) {
            ...
            if (is_executable(request_file)) {
                ...
                std::
                    cout << "Working execing script 3" << request_file <<
                    std::endl;
 ...
 for (const auto & item:parsed_json.items()) {
    ...
    static std::tuple < std::map < std::string, std::string >, std::string >
        ...
        return {
        split_query(query_string), query_string};
    };
    ...
    void set_socket_option() {
        ...
        try {
            set_sock_opt =
                setsockopt(this->_server_fd, SOL_SOCKET, SO_REUSEADDR, &opt,
                           sizeof(opt));
        }
    ...
    void bind_socket_to_address() {
        ...
        std::cout << "Server file descriptor: " << this->_server_fd << std::
            endl;
    };
